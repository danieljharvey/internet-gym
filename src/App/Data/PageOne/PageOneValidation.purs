...
validateState state = combineValidators [ isLikesDogs
                                        , isOverTen
                                        , badFirstName
                                        , badLastName
                                        ] state
...
isLikesDogs = validator (\s ->
  s.likesDogs == false) (const NoDogs)

isOverTen :: PageOneState -> Either (List PageOneError) PageOneState
isOverTen = validator (\s ->
  (compare s.age 10) == GT) (const TooYoung)

badFirstName :: PageOneState -> Either (List PageOneError) PageOneState
badFirstName = validator (\s ->
  not $ elem s.firstName badFirstNames) (\s ->
  BadFirstName s.firstName)

badLastName :: PageOneState -> Either (List PageOneError) PageOneState
badLastName = validator (\s ->
  not $ elem s.lastName badLastNames) (\s ->
  BadLastName s.lastName)
